...
function forEach(arr /*, limit, f*/) {
  var limit = arguments.length > 2 ? arguments[1] : Number.POSITIVE_INFINITY;
...
function map(arr /*, limit, f*/) {
  var limit = arguments.length > 2 ? arguments[1] : Number.POSITIVE_INFINITY;
  var f = arguments[arguments.length - 1];

  var results;
  asyncflow(limit, function() {
    results = arr.map(f).map(function(r) { return r.wait() });
  });
  return results;
}
